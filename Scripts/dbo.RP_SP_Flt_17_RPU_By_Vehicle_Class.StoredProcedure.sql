USE [GISData]
GO
/****** Object:  StoredProcedure [dbo].[RP_SP_Flt_17_RPU_By_Vehicle_Class]    Script Date: 2021-07-10 1:50:50 PM ******/
SET ANSI_NULLS ON
GO
...
MOD HISTORY:
Name 		Date		Comments
Roy He		2018-08-20  Acrrued
The RPU Report-  the total units are calucated
			  -  UT is calculated from scratch, not from stored data.	
*/
CREATE PROCEDURE [dbo].[RP_SP_Flt_17_RPU_By_Vehicle_Class]   --'*','*', '01 Jul 2018', '31 Jul 2018'
(
...
print @NumOfDays
 ...
 Select Distinct Revenue.*, 
		case when VU.NumberOfUnit is null 
			...
			--RentalDays,
			--RentalDaysWithinPeriod,
			--vu.TotalInserviceDays,
			RentalDaysWithinPeriod/vu.TotalInserviceDays VehicleUT,
			--RentalDays/
			--(
			--	(case when VU.NumberOfUnit is null 
			--		then 0
			--		else VU.NumberOfUnit 
			--	end)*Convert(decimal(9,2),DATEDIFF(Day,@startDate,@endDate+1))
			--) VehicleUT2,
			--(1.RentalDays_Curr/Average_Fleet_Curr*.BusinessDays)*100
...
(
	...
	(
		SELECT  	
	--                Hub_ID	Hub	Current_Location_ID	Current_Location_Name	Vehicle_Type_ID	Vehicle_Class_Code	Vehicle_Class_Name	Rentable	Not_Rentable	Rented	RP_Date
	--1                        	Vancouver	1	B-08 McDonald Road	Car	-	Premium 4X4	4	0	0	2013-08-01 03:08:03.027            
			...
			FROM 	Vehicle V
				...
				Left Join Vehicle_PullForDisposal_Periods_vw VPP
