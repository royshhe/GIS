USE [GISData]
GO
/****** Object:  StoredProcedure [dbo].[RP_SP_Con_2_Contract_Statistic_2_Year_Comparison_for All Location Total]    Script Date: 2021-07-10 1:50:50 PM ******/
SET ANSI_NULLS ON
GO
...
*/
/* updated to ver 80 */
CREATE PROCEDURE [dbo].[RP_SP_Con_2_Contract_Statistic_2_Year_Comparison_for All Location Total]-- '2011-08-01','2012-07-31','Car','*'
(
...
from 
	...
	--    		SUM(TimeKm_Charge) AS TimeKm_Charge, 
			...
			WHERE BRAC_Unit=1 
and Location_Name in (select location from location where owning_company_ID=(select code from lookup_Table where category='BudgetBC Company'))
...
and Location_Name in (select location from location where owning_company_ID=(select code from lookup_Table where category='BudgetBC Company'))
	...
	left outer join
			(select	Vehicle_Type_ID,
					sum(Loc_Average_Fleet_Curr) as Average_Fleet_Curr
				from (select	Vehicle_Type_ID, 
    					--Current_Location_ID, 
						...
						(cast(cast(sum(rentable)+sum(not_rentable)+sum(Rented) as decimal(11,4))
							...
							--* @AvgTotal_Curr
							--/ @AvgBracTotal_Curr  
							as decimal(11,4))) as Loc_Average_Fleet_Curr
					from RP_Flt_15_Vehicle_Utilization VU
					   WHERE 
						 rp_date BETWEEN dateadd(dd,1,CONVERT(datetime,  @paramStartDate+' 00:00:00')) AND dateadd(dd,1,CONVERT(datetime,  @paramEndDate+' 23:59:59')) 
						 AND ( @paramLocID='*' OR (VU.Current_Location_ID=CONVERT(INT,@tmpLocID) ) )
							 AND (@paramVehTypeID='*' OR (vu.Vehicle_Type_ID=@paramVehTypeID) )
	...
	left outer join
			(select Vehicle_Type_ID,
					sum(Loc_Average_Fleet_Pre) as Average_Fleet_Pre 
				from (select Vehicle_Type_ID, 
    					--Current_Location_ID, 
    					--Current_Location_Name, 
						(cast(cast(sum(rentable)+sum(not_rentable)+sum(Rented)as decimal(11,4))
							...
							--* @AvgTotal_Pre
							--/ @AvgBracTotal_Pre 
							as decimal(11,4))) as Loc_Average_Fleet_Pre
					from RP_Flt_15_Vehicle_Utilization VU
					   WHERE 
						 rp_date BETWEEN dateadd(dd,1,dateadd(yy,-1 ,CONVERT(datetime,  @paramStartDate+' 00:00:00')))
									 AND dateadd(dd,1,dateadd(yy,-1, CONVERT(datetime,  @paramEndDate+' 23:59:59')))	 
