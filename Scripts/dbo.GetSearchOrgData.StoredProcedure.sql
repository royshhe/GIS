USE [GISData]
GO
/****** Object:  StoredProcedure [dbo].[GetSearchOrgData]    Script Date: 2021-07-10 1:50:49 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[GetSearchOrgData]  --'ams' -- 'BUDGET OF IRELAND - 093916A'
@OrgName varchar(255)
...
--	And ACA.Customer_Code = ACM.Customer_Code
	...
	And (SubString(Price_Code, 1, 2) = 'DB' or Price_code in (select code from lookup_table where category='Direct Bill' ) or ACM.Customer_Code='BRACCRE')
	--and ACM.Customer_Code not in ('ONEPROD')
