USE [GISData]
GO
/****** Object:  View [dbo].[Reservation_Rates]    Script Date: 2021-07-10 1:50:45 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER OFF
GO

CREATE view [dbo].[Reservation_Rates]
as
select 	Res.Confirmation_number,
	rate.rate_id,
	rate.rate_name,
	rate.rate_level,
...
from Reservation_Change_History Res
inner join
(Select Distinct
	VR.Rate_Name,
	RCA.Rate_ID,
...
on rate.Rate_ID = res.Rate_id 
and rate.Rate_Level = res.Rate_level
and rate.Vehicle_Class_Code = res.Vehicle_Class_Code
and res.Date_Rate_Assigned between rate.Effective_Date and rate.Termination_Date
--where res.status not in ('VD', 'CA')
group by res.confirmation_number,
	rate.rate_id,
	rate.rate_name,
	rate.rate_level
...
on q.Quoted_Rate_id = res.Quoted_Rate_id
--where res.status not in ('VD', 'CA')
and 	res.Quoted_Rate_id is not null
where res.Quoted_Rate_id is not null
group by res.confirmation_number,
	q.Quoted_Rate_ID,
	q.rate_name
GO
