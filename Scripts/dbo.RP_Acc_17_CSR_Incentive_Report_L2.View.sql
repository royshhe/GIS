USE [GISData]
GO
/****** Object:  View [dbo].[RP_Acc_17_CSR_Incentive_Report_L2]    Script Date: 2021-07-10 1:50:44 PM ******/
SET ANSI_NULLS ON
GO
...
--			Roy He			2008-01-04      
---------------------------------------------------------------------------------------------------------------------
CREATE View [dbo].[RP_Acc_17_CSR_Incentive_Report_L2]
...
--------------------------------------------------------SALES-------------------------------------------------------------
    ...
    -- add logic for exclude complimentary contract for caculation rental days.  /peter 20121120  
    case when 	SUM( CASE 	WHEN Charge_Type IN (10, 11, 20,50, 51, 52)
							THEN Amount
							ELSE 0
						END) =0
			then 0
			else Contract_Rental_Days
		end  as Contract_Rental_Days,
	...
	SUM(CASE	WHEN Charge_Type = 34 and  ((Rate_Purpose<>'Tour Pkg'  or Rate_Purpose is Null)
			...
			-- Add Seat Storage
	SUM(Case	When Optional_Extra_ID in (1, 2, 3,76) and  ((Rate_Purpose<>'Tour Pkg'  or Rate_Purpose is Null)
											and (Applicant_Status_Indicator<>1 or Applicant_Status_Indicator is null)
											--or pick_up_location_id not in ('16','20')  
											)
			Then Amount
	...
	SUM(Case	When Optional_Extra_ID in (23, 25,229) --229 for GraduatedDriverSurcharge
			Then Amount
			ELSE 0
			END)						as Driver_Under_Age,
			
	--SUM(Case	When Optional_Extra_ID in (229) 
	--		Then Amount
	--		ELSE 0
	--		END)						as GraduatedDriverSurcharge,
...
--select * from contract_charge_item where contract_number=1202990
	...
	SUM(Case When OptionalExtraType IN ('LDW') --and Rate_Purpose<>'Tour Pkg' 
	...
	SUM(Case When OptionalExtraType IN ('BUYDOWN') --and Rate_Purpose<>'Tour Pkg' 
			Then Amount
			ELSE 0
			END)						as Buydown,
...
--			END)						as LDWQty,
	...
	SUM(Case	When Optional_Extra_ID in (47) 
			...
			ELSE 0
			END)						as Seat_Storage,
	SUM(Case	When Optional_Extra_ID in (50,51,52,72,73,74,75,248) 
				 OR (Charge_Type = 62 AND (Charge_Item_Type = 'a' Or Charge_Item_Type = 'm'))
			Then Amount
			ELSE 0
			END)						as Our_Of_Area,
...
-- --------------------------------------------Walk up-------------------------------------------------------------------
	...
	(case when ( Rate_Purpose<>'One Way' and  ((Rate_Purpose<>'Tour Pkg'  or Rate_Purpose is Null)
		...
		then     case when 	SUM( CASE 	WHEN Charge_Type IN (10, 11, 20,50, 51, 52)
							THEN Amount
							ELSE 0
						END) =0
					then 0
...
--A044300/'GOC%'   GOVERNMENT OF CANADA
	...
	and (Rate_Purpose<>'Special Events' or Rate_Purpose is null)--Kevin required for exclude Special Events rate on Jun 2015
