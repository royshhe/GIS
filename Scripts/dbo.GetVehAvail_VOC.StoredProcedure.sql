USE [GISData]
GO
/****** Object:  StoredProcedure [dbo].[GetVehAvail_VOC]    Script Date: 2021-07-10 1:50:49 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

/****** Object:  Stored Procedure dbo.GetVehAvail_VOC    Script Date: 2/18/99 12:12:17 PM ******/
/****** Object:  Stored Procedure dbo.GetVehAvail_VOC    Script Date: 2/16/99 2:05:43 PM ******/
/****** Object:  Stored Procedure dbo.GetVehAvail_VOC    Script Date: 1/11/99 1:03:17 PM ******/
/****** Object:  Stored Procedure dbo.GetVehAvail_VOC    Script Date: 11/23/98 3:55:34 PM ******/
CREATE PROCEDURE [dbo].[GetVehAvail_VOC]
	@LocationID	Varchar(10)
AS
	/* 3/08/99 - cpy bug fix - not returning vehicles if null values in
					do not rent past days/kms, etc. */
	/* 3/09/99 - cpy bug fix - put ISNULL check on Next_Scheduled_Maintenance */
	/* 3/12/99 - cpy bug fix - return foreign unit # if available, else return unit # */
...
DECLARE	@iBracCompanyId Int
	...
	FROM	Vehicle V
		...
		LEFT JOIN Vehicle_On_Contract VOC
		 ...
		 And VOC.Checked_out>VOC.Actual_Check_In
		LEFT JOIN Lookup_Table LT
		  ON V.Current_Condition_Status = LT.Code
		 AND LT.Category = 'Vehicle Condition Status'

	WHERE	V.Current_Location_Id = CONVERT(SmallInt, @LocationID)
	AND	V.Current_Vehicle_Status = 'd'		-- rental
