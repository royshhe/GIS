USE [GISData]
GO
/****** Object:  View [dbo].[RP_Con_4_Location_EOD_Reconciliation_SR_Cdn_Cash_Totals]    Script Date: 2021-07-10 1:50:45 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE VIEW [dbo].[RP_Con_4_Location_EOD_Reconciliation_SR_Cdn_Cash_Totals]
AS
-- Select Cdn Cash amounts for contracts
SELECT
	Business_Transaction.RBR_Date, 
	Business_Transaction.Location_ID, 
	lt3.Value AS Cash_Type,
	Debit_Card = CASE WHEN Cash_Payment.Cash_Payment_Type in (2, -3)  -- debit card or refund
			  THEN 1
			  ELSE 0
		     END,
	Cash = CASE WHEN Cash_Payment.Cash_Payment_Type IN (1, -2) -- Cash or Direct Refund (in cash)
                    THEN 1
		    ELSE 0
	       END,
	Cash_Cdn_Amount = CASE WHEN (Cash_Payment.Cash_Payment_Type > 0 )--cash payment or refund (refund positive)
...
FROM 	Contract WITH(NOLOCK)
	...
	JOIN
   	Cash_Payment 
		...
		AND Cash_Payment.Currency_ID =  1 -- Cdn$
...
SELECT 
	...
	Business_Transaction.Location_ID,
	lt3.Value AS Cash_Type,
	Debit_Card = CASE WHEN Reservation_Cash_Dep_Payment.Cash_Payment_Type in (2, -3)  -- debit card or refund
			  THEN 1
			  ELSE 0
		     END,
	Cash = CASE WHEN Reservation_Cash_Dep_Payment.Cash_Payment_Type IN (1, -2) -- Cash or Direct Refund (in cash)
                    THEN 1
		    ELSE 0
	       END,
	Reservation_Cash_Dep_Payment.Foreign_Currency_Amt_Collected AS Cash_Cdn_Amount --already negative for refund
...
FROM 	Business_Transaction 
	...
	JOIN
   	Reservation_Cash_Dep_Payment 
		...
		AND Reservation_Cash_Dep_Payment.Currency_ID =  1 -- Cdn$
...
SELECT 
	...
	Debit_Card = CASE WHEN Sales_Accessory_Cash_Payment.Cash_Payment_Type in (2, -3)  -- debit card or refund
			  THEN 1
			  ELSE 0
		     END,
	Cash = CASE WHEN Sales_Accessory_Cash_Payment.Cash_Payment_Type IN (1, -2) -- Cash or Direct Refund (in cash)
                    THEN 1
		    ELSE 0
	       END,
	Sales_Accessory_Cash_Payment.Foreign_Money_Collected AS Cash_Cdn_Amount --already negative for refund
...
FROM 	Business_Transaction 
	...
	JOIN
   	Sales_Accessory_Cash_Payment
		...
		AND Sales_Accessory_Cash_Payment.Currency_ID =  1 -- Cdn$
