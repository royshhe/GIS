USE [GISData]
GO
/****** Object:  StoredProcedure [dbo].[RP_SP_CCI_3_Credit_Card_Balancing_OM]    Script Date: 2021-07-10 1:50:50 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[RP_SP_CCI_3_Credit_Card_Balancing_OM]   --'2015-06-17'
(
...
--EIGEN TRANSACTIONS
  BEGIN
	...
	--CRLog.Credit_Card_Type CC_Type,
	...
	(
		...
		(
			...
			isnull(right(terminal_id,4)+left(rtrim(trn_id),12)+Authorization_Number,'') as  trn_id , 
		...
		(
			SELECT (case when dbo.Eigen_CRTransReport.trn_card_type<>'Other'
				  ...
				  right(Rtrim(dbo.Eigen_CRTransReport.Station_ID),4)+
					left(rtrim(Ltrim(dbo.Eigen_CRTransReport.trn_id)),12) +
					 rtrim(ltrim(dbo.Eigen_CRTransReport.trn_auth_code)) as OM_trn_id,
				  dbo.Eigen_CRTransReport.trn_datetime, 
				  cctrans.First_name+' '+cctrans.last_name as trn_card_owner,
				  '' as trn_ip, 
		...
		--					and right(dbo.Eigen_CRTransReport.trn_card_Expire,2)=left(cc.expiry,2)
					left join dbo.credit_card_transaction cctrans 
						on  --left(dbo.Eigen_CRTransReport.trn_card_number,6)=left(cctrans.Credit_Card_Number,6) and 
							right(dbo.Eigen_CRTransReport.trn_card_number,4)=right(cctrans.Credit_card_number,4) 
							and  left(dbo.Eigen_CRTransReport.trn_card_Expire,2)=right(cctrans.Expiry,2)
							and right(dbo.Eigen_CRTransReport.trn_card_Expire,2)=left(cctrans.expiry,2)
							and dbo.Eigen_CRTransReport.trn_auth_code=cctrans.authorization_number
							and left(rtrim(dbo.Eigen_CRTransReport.trn_id),12) =cctrans.trx_receipt_ref_num
			...
			where  (dbo.Eigen_CRTransReport.trn_datetime BETWEEN 
					...
					--and( dbo.Eigen_CRTransReport.rbr_date <='2010/05/02' and not ( dbo.Eigen_CRTransReport.trn_auth_code is null  or rtrim(ltrim(dbo.Eigen_CRTransReport.trn_auth_code))='')
					 --or dbo.Eigen_CRTransReport.rbr_date >'2010/05/02' 
					 and dbo.Eigen_CRTransReport.Response_Code<='010' 	 
