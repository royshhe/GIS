USE [GISData]
GO
/****** Object:  StoredProcedure [dbo].[GetUpdateStatusSearchData]    Script Date: 2021-07-10 1:50:49 PM ******/
SET ANSI_NULLS OFF
GO
...
--			Location L,
--			Lookup_Table LT1, Lookup_Table LT2, Lookup_Table LT3,  Vehicle_Last_Movement VLM
...
--			And LT1.Category = 'Vehicle Status'
--			And LT1.Code =* V.Current_Vehicle_Status
--			And LT2.Category = 'Vehicle Rental Status'
--			And LT2.Code =* V.Current_Rental_Status
--			And LT3.Category = 'Vehicle Condition Status'
--			And LT3.Code =* V.Current_Condition_Status
--			And V.Vehicle_Model_ID = VMY.Vehicle_Model_ID
--			And V.Current_Location_ID = L.Location_ID
--			And VLM.Unit_Number = V.Unit_Number
		Order By V.Unit_Number
	End
Else
	Begin
		Select Distinct
			V.Unit_Number, V.Foreign_Vehicle_Unit_Number, V.Serial_Number,
			VMY.Model_Year, VMY.Model_Name,
			LT1.Value, LT2.Value, LT3.Value,
			V.Current_Km, V.Ownership_Date, L.Location,
			V.Current_Licence_Plate,
...
--			Lookup_Table LT1, Lookup_Table LT2, Lookup_Table LT3,  Vehicle_Last_Movement VLM
		...
		Where
			V.Deleted=0
			And V.Current_Vehicle_Status <> 'a' /* Ignore Drop Ship Status */
			And V.Foreign_Vehicle_Unit_Number <> ''
			And NOT Foreign_Vehicle_Unit_Number IS NULL
			And V.Unit_Number >= ISNULL(@nBeginUnitNumber, V.Unit_Number)
			And V.Unit_Number <= ISNULL(@nEndUnitNumber, V.Unit_Number)
			And V.Current_Vehicle_Status = ISNULL(@CurrentStatusID, V.Current_Vehicle_Status)
...
--			And V.Current_Location_ID = L.Location_ID
--			And VLM.Unit_Number = V.Unit_Number
		Order By V.Foreign_Vehicle_Unit_Number
	End
Return 1
GO
