USE [GISData]
GO
/****** Object:  StoredProcedure [dbo].[RP_SP_Con_12_BCD_Monthly_Summary_Tracking]    Script Date: 2021-07-10 1:50:50 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[RP_SP_Con_12_BCD_Monthly_Summary_Tracking] -- '01 Jan 2007', '31 Dec 2007'
...
Select @SQLString='SELECT PU_Location, Month(RBR_Date) RBR_Month, count(*) Contract_Count,sum(ceiling(Contract_Rental_Days) ) as Rental_Days, sum(TimeCharge) as TimeCharge, sum(KMCharge) as KMCharge, sum(All_Level_LDW) as LDW, sum(PAI) as PAI, sum(PEC) as PEC,'
...
'FROM  dbo.[Contract_Revenue_BCD_Tracking_sum_vw]
