USE [GISData]
GO
/****** Object:  View [dbo].[RP_Acc_12_CSR_Incentive_Report_L2]    Script Date: 2021-07-10 1:50:45 PM ******/
SET ANSI_NULLS ON
GO
...
--
---------------------------------------------------------------------------------------------------------------------
CREATE View [dbo].[RP_Acc_12_CSR_Incentive_Report_L2]
...
SELECT
	...
	Reservation_Revenue,
  	/*(case 
		when Km_Cap=0 
			then dbo.GetRentalDays(Contract_Rental_Hours)*Per_KM_Charge*100
				else 0
		end 
               )  as ResKMCharge,
             */
	
	/* Reservation_Revenue=ResRevenue+
			(case 
				when Km_Cap=0 
				then Contract_Rental_Days*Per_KM_Charge*100
 				else 0
			end 
                                         )  ,*/
	/*FPORate,
	...
	case when	Location_Fee_Included = 1
			then sum(case when Charge_type = 10
					then amount
					else 0
					end) * (Percentage_Fee/100)
			else 0
			end						as Location_fee_amount,
	case when	License_Fee_Included = 1
			then (ceiling(Contract_Rental_Days) * LicenseFeePerDay)
			else 0
			end						as License_fee_amount,
	 
 	 
	-- Calculation Without Hours
 	SUM( CASE 	WHEN Charge_Type IN (10) and Unit_Type<>'Hour'
			THEN Amount
			ELSE 0				
			END)  	
	+
	...
	-- Calculation With Hours 						
           /* SUM( CASE 	WHEN Charge_Type IN (10,  50, 51, 52)
			THEN Amount
			ELSE 0
			END)             as Rental_Time_Revenue1,
             */ 

	SUM( CASE 	WHEN Charge_Type IN (11)
			THEN Amount
			ELSE 0
			END)  						as Rental_KM_Charge,
	...
	SUM(Case When OptionalExtraType IN ('PAI','PAI')
