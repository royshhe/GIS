USE [GISData]
GO
/****** Object:  StoredProcedure [dbo].[CreateVehicle_No_Identity]    Script Date: 2021-07-10 1:50:48 PM ******/
SET ANSI_NULLS OFF
GO
...
CREATE PROCEDURE [dbo].[CreateVehicle_No_Identity]
...
If @VehicleModelID IS NULL 		/* The Model,ModelYear pair is invalid */
	Return -1
Select @BudgetBCID = (Select CONVERT(Int, Code)
			From Lookup_Table
			Where Category='BudgetBC Company')

Select @thisUnitNumber = CONVERT(Int, NULLIF(@UnitNumber, ''))

-- Turn off identity 
Set Identity_Insert Vehicle ON

Insert Into Vehicle
	(Unit_Number,  Serial_Number, MVA_Number,Key_Ignition_Code, Key_Trunk_Code,
	 ...
	 Licence_Plate_Attached_On, Comments, Risk_Type, Deleted, Owning_Company_ID,
	 Drop_ShipDate, Current_Licencing_Prov_State,
...
Values
	(@thisUnitNumber,	 
	 ...
	 NULLIF(@TransferDate,''), @Comments, NULLIF(@RiskType, ''),0, @BudgetBCID,
...
-- insert a record into vehicle history table
...
If @PlateNumber <> ''
	Begin
		-- insert a record into vehicle licence history table
