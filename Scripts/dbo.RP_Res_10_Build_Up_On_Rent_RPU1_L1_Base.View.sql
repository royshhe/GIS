USE [GISData]
GO
/****** Object:  View [dbo].[RP_Res_10_Build_Up_On_Rent_RPU1_L1_Base]    Script Date: 2021-07-10 1:50:45 PM ******/
SET ANSI_NULLS ON
GO
...
/*
VIEW NAME:  	RP_Res_10_Build_Up_On_Rent_RPU1_L1_Base
PURPOSE:    	This view counts number of Reservation Pick Up for BRAC (local) and foreign locations
   		(Reservation with status of active and pick up date = date of day being counted) in the
		next 45 days.
...
CREATE VIEW [dbo].[RP_Res_10_Build_Up_On_Rent_RPU1_L1_Base]
AS
SELECT	CONVERT(datetime, CONVERT(varchar(12), Reservation.Pick_Up_On, 112)) AS Rpt_Date, 
...
WHERE	Reservation.Status = "A"
	AND DATEDIFF(dd, GETDATE(), Reservation.Pick_Up_On) >= 0 
