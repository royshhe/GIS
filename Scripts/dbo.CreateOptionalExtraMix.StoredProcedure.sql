USE [GISData]
GO
/****** Object:  StoredProcedure [dbo].[CreateOptionalExtraMix]    Script Date: 2021-07-10 1:50:47 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[CreateOptionalExtraMix]         -- '*', '*'
(
...
select @ReportDate= max(rbr_date) from rbr_date where Budget_Close_Datetime is not null
-- Creteria: Pickup Location, Optional Extra Type
				 --Optional Extra Inventory Tracking......
Delete Optional_Extra_Mix where Rpt_Date=@ReportDate

Insert Into Optional_Extra_Mix
Select 

@ReportDate as Rpt_Date,
OPT.Optional_Extra_ID,
OPT.Optional_Extra,
OPT.CurrentLoc as Current_Location,
SUM(Case when OPT.Status= 'On Rent' Then 1 Else 0 End) as On_Rent,
SUM(Case when OPT.Status='Available' Then 1 Else 0 End) as Available
...
And OPT.Optional_Extra in ('GPS', 'Infant Seat', 'Booster Seat','Child Seat')
Group by 
--@ReportDate,
OPT.Optional_Extra_ID,
OPT.Optional_Extra,
