...
------------------------------------------------------------------------- */
(function($) {
	$.lightbox = {version: '3.1.3'};
	
	$.fn.lightbox = function(pp_settings) {
		pp_settings = jQuery.extend({
			...
			deeplinking: true, /* Allow lightbox to update the url to enable deeplinking. */
			overlay_gallery: true, /* If set to true, a gallery will overlay the fullscreen image on mouse over */
			keyboard_shortcuts: true, /* Set to false if you open forms inside lightbox */
		...
		$(window).unbind('resize.lightbox').bind('resize.lightbox',function(){ _center_overlay(); _resize_overlay(); });
		
		if(pp_settings.keyboard_shortcuts) {
			$(document).unbind('keydown.lightbox').bind('keydown.lightbox',function(e){
		...
		*/
		$.lightbox.initialize = function() {
			...
			if(settings.allow_resize)
				$(window).bind('scroll.lightbox',function(){ _center_overlay(); });
		...
		*/
		$.lightbox.startSlideshow = function(){
			if(typeof pp_slideshow == 'undefined'){
				...
				pp_slideshow = setInterval($.lightbox.startSlideshow,settings.slideshow);
			}else{
				$.lightbox.changePage('next');	
			};
		}
		...
		*/
		function _showContent(){
			...
			$pp_pic_holder.animate({
				...
				if(settings.autoplay_slideshow && !pp_slideshow && !pp_open) $.lightbox.startSlideshow();
		...
		*/
		...
		function _build_overlay(caller){
			...
			$('a.pp_close').bind('click',function(){ $.lightbox.close(); return false; });
		...
		if(!pp_alreadyInitialized && getHashtag()){
			...
			// Little timeout to make sure all the lightbox initialize scripts has been run.
			// Useful in the event the page contain several init scripts.
			setTimeout(function(){ $("a[rel^='"+hashRel+"']:eq("+hashIndex+")").trigger('click'); },50);
		}
		
		return this.unbind('click.lightbox').bind('click.lightbox',$.lightbox.initialize); // Return the jQuery object for chaining. The unbind method is used to avoid click conflict when the plugin is called more than once
	};
	...
	function clearHashtag(){
		// Clear the hashtag only if it was set by lightbox
		url = location.href;
		hashtag = (url.indexOf('#!lightbox')) ? true : false;
		if(hashtag) location.hash = "!lightbox";
	}
